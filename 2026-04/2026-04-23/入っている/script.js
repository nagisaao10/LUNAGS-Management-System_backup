...
function renderRailList(data, selector) {
  const container = document.querySelector(selector); // ← これ絶対必要
  if (!container) return;
...
});});tem) => `<div class="list-item"><a href="${item.link || "#"}"><span class="list-date">${getDate(item).toLocaleDateString()} ${getDate(item).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span><p class="list-title">${item.title} / ${item.member}</p></a></div>`
  );
